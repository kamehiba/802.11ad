...
main (int argc, char *argv[])
{
  ...
  for (uint16_t i = 0; i < numberOfUes; i++)
    {
      positionAlloc->Add (Vector (-100, 20, 0));
    }
  ...
  ueIpIfaces = epcHelper->AssignUeIpv4Address (NetDeviceContainer (enbLteDevs));
...
//          lteHelper->ActivateDedicatedEpsBearer (ueLteDevs.Get (u), bearer, tft);
  ...
  lteHelper->AddX2Interface (ueNodes);

  // X2-based Handover
  lteHelper->HandoverRequest (Seconds (0.900), ueLteDevs.Get (0), enbLteDevs.Get (0), enbLteDevs.Get (2));
